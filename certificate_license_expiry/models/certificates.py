...
class Certificates(models.Model):
    ...
    state = fields.Selection(
        ...
        string="State", default='new', track_visibility="onchange", help="States of certificate")
    ...
    certificates_types_id = fields.Many2one('certificates.types',
                                            ...
                                            required=True, help="Type of the certificate")
    start_date = fields.Date(string="Start Date", required=True,
                             default=fields.Date.today(), help="Certificate start date")
    expire_date = fields.Date(string="Expire Date", help="Certificate expiry date")
    issued_company_id = fields.Many2one('res.company', string="Issued By",
                                        required=True,
                                        help="Certificate issued by which company")
    certificates_tags_ids = fields.Many2many('certificates.tags', string="Tags",
                                             help="Tags of the certificate")
    project_id = fields.Many2one('project.project', string="Project",
                                 required=True,
                                 help="Project corresponding to the certificate")
    ...
    product_id = fields.Many2one('product.product', string="Product",
                                 required=True,
                                 help="Product corresponding to the certificate")
    ...
    company_id = fields.Many2one('res.company', string="Company",
                                 ...
                                 readonly=True,
                                 help="Company corresponding to the certificate")
    expire_remainder_day = fields.Integer(string="Expire Reminder Day",
                                          help="Certificate expire remainder day")
    ...
    has_expired = fields.Boolean(string="Expired Certificate",
                                 help="Becomes True if the certificate has expired")
    access_token = fields.Char("Security Token", copy=False)
    developer_document1 = fields.Binary(string="Developer Document 1", 
                                    help="Upload developer document 1")
    developer_document1_filename = fields.Char(string="Developer Document 1 Filename")
    developer_document2 = fields.Binary(string="Developer Document 2", 
                                        help="Upload developer document 2")
    developer_document2_filename = fields.Char(string="Developer Document 2 Filename")
    signed_document1 = fields.Binary(string="Signed Document 1", 
                                    help="Upload signed and verified document 1")
    signed_document1_filename = fields.Char(string="Signed Document 1 Filename")
    signed_document2 = fields.Binary(string="Signed Document 2", 
                                    help="Upload signed and verified document 2")
    signed_document2_filename = fields.Char(string="Signed Document 2 Filename")
    document_request_sent = fields.Boolean("Document Request Sent", default=False)
    document_request_date = fields.Datetime("Document Request Date")
    developer_doc1_uploaded = fields.Boolean("Developer Doc 1 Uploaded", default=False)
    developer_doc2_uploaded = fields.Boolean("Developer Doc 2 Uploaded", default=False)
    signed_doc1_uploaded = fields.Boolean("Signed Doc 1 Uploaded", default=False)
    signed_doc2_uploaded = fields.Boolean("Signed Doc 2 Uploaded", default=False)

    def _compute_access_url(self):
        """Inherited from portal.mixin to set specific access URL"""
        super(Certificates, self)._compute_access_url()
        for certificate in self:
            certificate.access_url = '/my/certificate/%s' % certificate.id

    def _portal_ensure_token(self):
        """Generate an access token if not already done"""
        for record in self:
    ...
    def action_send_document_request(self):
        """Send document upload link to the partner"""
        self.ensure_one()
        template = self.env.ref('certificate_license_expiry.email_template_document_request')
        ...
        self.write({
            ...
            'document_request_date': fields.Datetime.now(),
        })
        ...
        return {
            ...
            'params': {
                'title': _('Document Request Sent'),
                'message': _('Document upload link has been sent to %s') % self.customer_id.email,
                'type': 'success',
                'sticky': False,
            }
        }
