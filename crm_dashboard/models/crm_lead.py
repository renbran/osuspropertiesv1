...
class CRMLead(models.Model):
    ...
    @api.model
    def get_department_performance(self, kwargs):
        """Get department-wise performance metrics
        Returns:
            dict: Department performance data with deals, sales, commission, avg deal size
        """
        # Commission rate (adjust based on business logic)
        COMMISSION_RATE = 0.06  # 6% commission
        
        departments = {
            'exclusive': {
                'name': 'Exclusive Sales',
                'team_ids': [],  # Can be filtered by team_id if needed
            },
            'primary': {
                'name': 'Primary Sales',
                'team_ids': [],
        ...
        for dept_key, dept_info in departments.items():
            domain = [
                ('stage_id', '=', won_stage.id if won_stage else False),
                ('type', '=', 'opportunity'),
            ]
            
            # If you have team filtering, add here:
            # if dept_info['team_ids']:
            #     domain.append(('team_id', 'in', dept_info['team_ids']))
            
            leads = self.search(domain)
            
            total_deals = len(leads)
            total_value = sum(lead.expected_revenue for lead in leads)
