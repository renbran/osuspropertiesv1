...
class CommissionReportGenerator(models.Model):
    ...
    def _generate_partner_statement_pdf(self, commission_data, wizard_data):
        """Generate PDF partner statement report with professional formatting"""
        ...
        story.append(Paragraph("Professional Commission Management Report", subtitle_style))
        story.append(Paragraph(
            f"Report Period: {wizard_data.get('date_from', 'N/A')} to {wizard_data.get('date_to', 'N/A')}", 
            period_style
        ))
        ...
        # Create a compact summary table with two columns of metrics
        summary_data = [
            [
                Paragraph('<b>Metric</b>', cell_center_style),
                Paragraph('<b>Value</b>', cell_center_style),
                Paragraph('<b>Metric</b>', cell_center_style),
                Paragraph('<b>Value</b>', cell_center_style),
            ],
            [
                Paragraph('Total Records', cell_style),
                Paragraph(str(totals['commission_count']), cell_right_style),
                Paragraph('Total Sales', cell_style),
                Paragraph(f"{primary_currency} {totals['total_sales']:,.2f}", cell_right_style),
            ],
            [
                Paragraph('Total Commissions', cell_style),
                Paragraph(f"{primary_currency} {totals['total_commissions']:,.2f}", cell_right_style),
                Paragraph('External Commissions', cell_style),
                Paragraph(f"{primary_currency} {totals['external_commissions']:,.2f}", cell_right_style),
            ],
            [
                Paragraph('Internal Commissions', cell_style),
                Paragraph(f"{primary_currency} {totals['internal_commissions']:,.2f}", cell_right_style),
        ...
        summary_table = Table(
            summary_data, 
            colWidths=[1.6*inch, 1.3*inch, 1.6*inch, 1.3*inch]
        )
        summary_table.setStyle(TableStyle([
            ...
            # Grid
            ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#800020')),
        ...
        if commission_data['report_data']:
            # Headers with sequence number
            headers = [
                Paragraph('<b>#</b>', cell_center_style),
                Paragraph('<b>SO Reference</b>', cell_center_style),
                Paragraph('<b>Date</b>', cell_center_style),
                Paragraph('<b>Project</b>', cell_center_style),
                Paragraph('<b>Unit</b>', cell_center_style),
                Paragraph('<b>Percent</b>', cell_center_style),
                Paragraph('<b>Sales Value</b>', cell_center_style),
                Paragraph('<b>Total Amount</b>', cell_center_style),
                Paragraph('<b>Status</b>', cell_center_style),
            ]
            table_data = [headers]
            
            # Add data rows with sequence numbers
            for idx, line in enumerate(commission_data['report_data'], start=1):
                ...
                # Truncate long project names for better fit
                ...
                row = [
                    ...
                    Paragraph(line['sale_order_name'], cell_style),
                    Paragraph(str(line['booking_date']) if line['booking_date'] else 'N/A', cell_center_style),
                    Paragraph(project_name, cell_style),
                    Paragraph(unit_name, cell_center_style),
                    Paragraph(f"{line['commission_rate']:.2f}%", cell_center_style),
                    Paragraph(f"{currency} {line['unit_price']:,.2f}", cell_right_style),
                    Paragraph(f"{currency} {line['commission_amount']:,.2f}", cell_right_style),
                    Paragraph(line['commission_status'], cell_center_style),
                ]
                table_data.append(row)
            
            # Column widths adjusted for sequence number column
            col_widths = [
                0.35*inch,   # # (sequence)
                0.75*inch,   # SO Reference
                0.7*inch,    # Date
                1.4*inch,    # Project
                0.65*inch,   # Unit
                0.55*inch,   # Percent
                1.0*inch,    # Sales Value
                0.95*inch,   # Total Amount
                0.7*inch,    # Status
            ]
            
            detail_table = Table(table_data, repeatRows=1, colWidths=col_widths)
            
            # Build alternating row colors list
            ...
            table_style_commands = [
                ...
                ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                # Alignment overrides
                ('ALIGN', (0, 1), (0, -1), 'CENTER'),   # Sequence - center
                ('ALIGN', (1, 1), (1, -1), 'LEFT'),     # SO Reference - left
                ('ALIGN', (2, 1), (2, -1), 'CENTER'),   # Date - center
                ('ALIGN', (3, 1), (3, -1), 'LEFT'),     # Project - left
                ('ALIGN', (4, 1), (4, -1), 'CENTER'),   # Unit - center
                ('ALIGN', (5, 1), (5, -1), 'CENTER'),   # Percent - center
                ('ALIGN', (6, 1), (6, -1), 'RIGHT'),    # Sales Value - right
                ('ALIGN', (7, 1), (7, -1), 'RIGHT'),    # Total Amount - right
                ('ALIGN', (8, 1), (8, -1), 'CENTER'),   # Status - center
            ]
            ...
            for row_idx in range(1, row_count):
                if row_idx % 2 == 0:
                    table_style_commands.append(
                        ('BACKGROUND', (0, row_idx), (-1, row_idx), colors.HexColor('#FDF5F5'))
                    )
                else:
                    table_style_commands.append(
            ...
            story.append(Paragraph("No commission data found for the selected criteria.", no_data_style))
        ...
        if 'categories' in commission_data:
            ...
            for category, data in commission_data['categories'].items():
                category_data.append([
                    ...
                    Paragraph(f"${data['total_amount']:,.2f}", category_value_style),
